...
export async function onRequestGet(context) {
    ...
    try {
        ...
        headers.append('Location', baseUrl + '/?' + redirectParams.toString());
        headers.append('Set-Cookie', 'gh_oauth_state=; Path=/; HttpOnly; Max-Age=0');
        headers.append('Set-Cookie', 'gh_oauth_token=' + encryptedToken + '; Path=/; HttpOnly; Max-Age=86400; SameSite=Lax; Secure');
        return new Response(null, { status: 302, headers });
    } catch (e) {
        const detail = encodeURIComponent(String((e && e.message) ? e.message : e));
        return Response.redirect(baseUrl + '/?oauth_error=github_exception&detail=' + detail, 302);
    }
}
