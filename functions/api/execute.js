...
// Read a fetch Response safely: parse JSON if possible, otherwise return the
// raw text. This prevents crashes when GitHub returns a plain-text error like
// "Request forbidden by administrative rules" instead of JSON.
...
function ghError(b, fallback) {
  if (b.json && b.json.message) return b.json.message;
  if (b.raw && b.raw.trim()) return b.raw.trim().slice(0, 300);
  return fallback + ' (HTTP ' + b.status + ')';
}
...
export async function onRequestPost(context) {
  ...
  try {
    if (action === 'list_repos') {
      ...
      if (!b.ok || !Array.isArray(b.json)) return Response.json({ error: ghError(b, 'Could not list repos.') }, { status: 500 });
      const repos = b.json.map(rp => ({ name: rp.name, full_name: rp.full_name, pushed_at: rp.pushed_at?.slice(0, 10) }));
    ...
    if (action === 'read_file_excerpt') {
      ...
      const b = await readBody(r);
      if (!b.ok || !b.json || !b.json.content) return Response.json({ error: ghError(b, `File not found: ${path} in ${repo}`) }, { status: b.status || 404 });
      const full = base64Decode(b.json.content);
      const excerpt = full.length > 6000
        ? full.slice(0, 3000) + '\n\n...[TRUNCATED — file is ' + full.length + ' chars]...\n\n' + full.slice(-1500)
        : full;
      return Response.json({ excerpt, sha: b.json.sha, size: full.length });
    }
    ...
    if (action === 'inject_html') {
      ...
      let fileText = base64Decode(b.json.content);
      ...
      const pb = await readBody(pushRes);
      if (pb.ok && pb.json && pb.json.commit) {
        return Response.json({
          success: true,
          commit: pb.json.commit.sha,
          repo,
          path,
          message: `✅ Pushed to ${repo}/${path} — Cloudflare will redeploy in ~30 seconds.`
        });
      }
      return Response.json({ error: 'GitHub write failed: ' + ghError(pb, 'GitHub push failed.') }, { status: pb.status || 500 });
    }
    ...
    if (action === 'push_file') {
      ...
      const pushBody = { message: message || 'Update file via Neocryptz AI', content: base64Encode(content) };
      ...
      if (pb.ok && pb.json && pb.json.commit) {
        return Response.json({
          success: true,
          commit: pb.json.commit.sha,
          message: `✅ Pushed to ${path} — Cloudflare will redeploy in ~30 seconds.`
        });
      }
      return Response.json({ error: 'GitHub write failed: ' + ghError(pb, 'GitHub push failed.') }, { status: pb.status || 500 });
    }
    ...
    if (action === 'redeploy') {
      ...
      if (deployData.success) {
        return Response.json({ success: true, deployment_id: deployData.result?.id, message: '✅ Redeploy triggered — Cloudflare is building the latest commit now.' });
      }
